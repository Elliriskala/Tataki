...
const fetchUserOrders = async (
  ...
  try {
    const sql = `
            ...
            ORDER BY Orders.created_at DESC;
        `;
    const [rows]: any = await promisePool.query(sql, [userId]);
    if (rows && rows.length > 0) {
      return rows;
    }
    return null;
...
const fetchOrderById = async (
  ...
  try {
    const sql = `
            ...
            ORDER BY Orders.created_at DESC;
        `;
    const [rows]: any = await promisePool.query(sql, [orderId]);
    if (rows && rows.length > 0) {
      return rows[0];
    }
    return null;
...
const fetchOrdersByStatus = async (
  ...
  try {
    const sql = `
                ...
                ORDER BY Orders.created_at DESC;
            `;
    const [rows]: any = await promisePool.query(sql, [orderStatus]);
    if (rows && rows.length > 0) {
      return rows;
    }
    return null;
...
const createOrder = async (
  ...
  try {
    ...
    const orderItemsSql =
      "INSERT INTO order_items (order_id, menu_id, item_quantity, comment) VALUES ?";
