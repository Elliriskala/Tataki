...
const itineraryForm = document.getElementById(
  "itineraryForm"
) as HTMLFormElement;
const itineraryResults = document.getElementById(
  "itineraryResults"
) as HTMLDivElement;
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      async function (position) {
        ...
        const fromLatElement = document.getElementById(
          "fromLat"
        ) as HTMLInputElement;
        const fromLonElement = document.getElementById(
          "fromLon"
        ) as HTMLInputElement;
    ...
    alert(
      "Geolocation not supported. Please switch your browser or refresh the page."
    );
  }
});
...
itineraryForm?.addEventListener("submit", async (event) => {
  ...
  const fromLat = fromLatElement
    ? (fromLatElement as HTMLInputElement).value
    : "";
  const fromLonElement = document.getElementById("fromLon") as HTMLInputElement;
  const fromLon = fromLonElement
    ? (fromLonElement as HTMLInputElement).value
    : "";
  ...
  const walkSpeedElement = document.getElementById(
    "walkSpeed"
  ) as HTMLInputElement;
  const walkSpeed = walkSpeedElement
    ? (walkSpeedElement as HTMLInputElement).value
    : "";
